...
const ChatView: React.FC<ChatViewProps> = ({ messages, setMessages, apiKey, onApiError, profile }) => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (error?.message?.includes('429') || error?.message?.includes('401')) {
        alert("API Key expired or credit exhausted. Please provide a new key.");
        onApiError();
      } else {
